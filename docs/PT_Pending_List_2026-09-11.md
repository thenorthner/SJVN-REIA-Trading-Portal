...
| 46 ✅ | **Teen screens ki sample-data notice check** | **14 Sep.** **IEX** aur **PXIL** ki notice **sahi hai aur khud sudharti hai** — wo server ke `mode: STUB|LIVE` par chalti hai, credentials lagte hi apne aap gayab ho jaayegi; kuch badla nahi. **Certificate Operations Hub** ki notice sach thi par wajah galat: screen `Math.random()` se bid book banati thi jabki `rec_bids`, `escert_orders` aur REC ledger database mein padhe the. Ab: bid book asli register se (REC → rec_bids, ESCert → escert_orders), holdings REC ledger se (held / committed / sellable) aur ESCert ka figure "configured" label ke saath, price band master data se, aur **Submit Bid ab sach mein bid raise karta hai** (pehle alert deta tha "nothing was saved" — jabki endpoint maujood tha). Registry / Obligation / Tax-invoice views ka koi store nahi hai — ab wo ye saaf bolti hain, nakli rows ki jagah. 6 test. |
| 45 ✅ | **DOR aur Schedule Archive ab asli data par** | **Fix, 14 Sep.** `/api/trading/dor` 96 block ek likhi hui curve se banata tha, total revenue hardcoded ("for screenshot replica"), aur ek signatory ka naam jo yahan kaam hi nahi karta — jo bhi date poochho, jawab wahi. Ab bid book se banta hai: jis block par bid clear hui wahi block, uska cleared price, aur value; kuch nahi to saaf likhta hai "is date par koi obligation nahi". Screen se wo settlement waterfall (NLDC/CTU/STU/SLDC/IEX fee/IGST/net payout + "MATCHED WITH IEX PORTAL" badge), SAP journal modal aur nakli IEX PDF viewer hata diye — ye charges exchange invoice par uthte hain. `/api/trading/archive` 30 nakli filename banata tha; ab `csv_uploads` register padhta hai (file, RLDC, period, revision, rows, kisne upload ki). 10 test. |
| 44 ✅ | **ERP Vendor Format ab asli vendor list se** | **Fix, 14 Sep.** FLVN00 extract un logon ki list hai jinhe SJVN pay karta hai, aur wo list platform ke paas hai — TDS register (`tds_vendors`), jispe deduction entries tikti hain. Ab wahi se banta hai (naam, PAN search term mein, category = vendor type, inactive vendor bahar), aur register khaali ho to paste kiya hua fallback. |
