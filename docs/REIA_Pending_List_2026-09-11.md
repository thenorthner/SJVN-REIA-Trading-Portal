...
| 12 ✅ | **Peak availability penalty** | H, J | **Fix, 14 Sep.** Contract par peak window (from/to, aadhi raat paar bhi chalta hai), min peak availability % aur peak penalty ₹/MWh; energy data mein us mahine ka **peak-window availability %** (REA/JMR jo batata hai — platform ke paas block-level data nahi hai, monthly hi hai). Bill par: shortfall % × capacity × peak hours × rate, CUF penalty ke saath. **Jis contract par peak obligation nahi hai uspar kuch charge nahi hota** — na chup-chaap zero, na koi default. Amendment naya version row likhta hai, wo bhi obligation le kar jaata hai (test isi ke liye hai). 10 test. |
