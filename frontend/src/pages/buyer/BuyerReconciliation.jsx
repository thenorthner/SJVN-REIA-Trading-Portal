...
export default function BuyerReconciliation() {
  ...
  function triggerAck(decision) {
    setAckModal({
      open: true,
      decision,
      remarks: decision === 'AGREE' ? 'Verified with Discom billing schedules and energy ledger.' : '',
      confirmed: false,
    });
  }
  ...
  async function submitAck() {
    ...
    try {
      ...
      setAckFeedback({
        ...
        title: isAgree ? '✅ Discom Sign-off Recorded!' : '⚠️ Disagreement Recorded & Disputed',
        message: res.message || (isAgree ? 'Discom joint sign-off successfully logged with timestamp.' : 'Disagreement recorded.'),
      });
  ...
  return (
    <div>
            ...
            <div style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <div style={{ padding: 10, background: detail.counterparty_ack_at ? '#ecfdf5' : '#ffffff', border: `1px solid ${detail.counterparty_ack_at ? '#a7f3d0' : 'var(--border)'}`, borderRadius: 6 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: detail.counterparty_ack_at ? '#065f46' : 'var(--text-muted)' }}>🏢 Your Discom Sign-off</div>
                  <div style={{ fontSize: 12, marginTop: 3 }}>
      ...
      {/* Modal: Interactive Buyer Acknowledgment / Disagreement */}
      <Modal
        open={ackModal.open}
        onClose={() => !ackLoading && setAckModal({ ...ackModal, open: false })}
        title={ackModal.decision === 'AGREE' ? '✍️ Discom Digital Sign-off & Acknowledgment' : '⚠️ Disagreement & Dispute Notice'}
        width={540}
      >
        <div>
          {ackModal.decision === 'AGREE' ? (
            <div>
              <div style={{ background: '#ecfdf5', border: '1px solid #a7f3d0', borderRadius: 8, padding: 14, marginBottom: 14, color: '#065f46', fontSize: 13 }}>
                ...
                <div><strong>Contract:</strong> {detail?.contract_no}</div>
                <div><strong>Signatory:</strong> {user?.name} (Buyer Discom)</div>
              </div>
              ...
              <Field label="Sign-off Remarks (optional)">
                <input
                  ...
                  placeholder="e.g. Verified and approved by Discom commercial division."
                />
              </Field>
              ...
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginTop: 14, fontSize: 13, cursor: 'pointer', background: 'var(--surface)', padding: 10, borderRadius: 6, border: '1px solid var(--border)' }}>
                ...
                />
                <span>
                  <strong>I confirm Discom sign-off</strong>: I have reviewed the energy billing statement for period <strong>{detail?.period}</strong> and formally agree with this reconciliation.
                </span>
              </label>
            ...
            <div>
              ...
              <Field label="Disagreement Reason / Discrepancy Details *">
                <textarea
                  ...
                  value={ackModal.remarks}
                  onChange={(e) => setAckModal({ ...ackModal, remarks: e.target.value })}
                  placeholder="State the discrepancy in MWh energy units, tariff, or payment credit..."
                  style={{ width: '100%' }}
                />
              </Field>
