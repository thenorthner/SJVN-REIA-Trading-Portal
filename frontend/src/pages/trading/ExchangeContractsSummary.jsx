...
function rowsForExport(rows) {
  return rows.map((r) => ({
    'Contract No': r.loa_no || r.id,
    'Registration Type': r.contract_type || 'Exchange',
...
export default function ExchangeContractsSummary() {
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      list = rows.filter((r) => {
        const hay = [r.loa_no, r.ppa_no, r.contract_type, r.client_name, r.id, fmtCreated(r.created_at)]
          .filter(Boolean)
          .join(' ')
          .toLowerCase();
  ...
  function exportCsv() {
    ...
    downloadBlob(`exchange-contracts-${new Date().toISOString().slice(0, 10)}.csv`, new Blob([lines.join('\n')], { type: 'text/csv' }));
  }
  ...
  function exportExcel() {
    ...
    XLSX.utils.book_append_sheet(wb, sheet, 'Exchange Contracts');
    XLSX.writeFile(wb, `exchange-contracts-${new Date().toISOString().slice(0, 10)}.xlsx`);
  }
  ...
  function exportPdf() {
    ...
    w.document.write(`<!doctype html><html><head><title>Exchange Contracts</title>
      <style>
  ...
  return (
    <div style={{ padding: 20 }}>
      <div className="form-section-header" style={{ marginTop: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span>Exchange Contracts</span>
        <button type="button" className="btn btn-sm btn-primary" onClick={() => navigate('/trading/exchange/create')}>
          + Create Contract
        </button>
      </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap', marginBottom: 14 }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
            ...
            <input
              ...
              placeholder="Contract no, PPA, client…"
            />
          </label>
        </div>
                  ...
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={4} style={{ padding: 24, textAlign: 'center', color: '#64748b' }}>
                        No exchange contracts yet.{' '}
                        <Link to="/trading/exchange/create">Create one</Link>
                      </td>
                    </tr>
                  ) : filtered.map((r) => (
                    <tr key={r.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                      <td style={{ padding: '10px 12px' }}>
                        <Link to={`/trading/exchange/contracts/${r.id}`} style={{ color: '#1d4ed8', fontWeight: 500 }}>
                          {r.loa_no || r.id}
                        </Link>
                      </td>
                      <td style={{ padding: '10px 12px' }}>{r.contract_type || 'Exchange'}</td>
