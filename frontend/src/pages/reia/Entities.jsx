...
function InvoiceConfigForm({ initialJson, onSave }) {
  const [data, setData] = useState(() => {
    try {
      return JSON.parse(initialJson || '{"logo_url":"", "signatory_name":"", "signatory_designation":"", "address_override":""}');
    } catch {
      return { logo_url: '', signatory_name: '', signatory_designation: '', address_override: '' };
    }
  });
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="form-grid">
        <Field label="Logo URL"><input value={data.logo_url || ''} onChange={e => setData({...data, logo_url: e.target.value})} placeholder="https://..." /></Field>
        <Field label="Signatory Name"><input value={data.signatory_name || ''} onChange={e => setData({...data, signatory_name: e.target.value})} /></Field>
        <Field label="Signatory Designation"><input value={data.signatory_designation || ''} onChange={e => setData({...data, signatory_designation: e.target.value})} /></Field>
        <Field label="Address Override"><input value={data.address_override || ''} onChange={e => setData({...data, address_override: e.target.value})} placeholder="Leave blank to use default entity address" /></Field>
      </div>
        ...
        <Modal open={true} onClose={() => setSelected(null)} title={`Stakeholder: ${selected.name}`} width={800}>
          ...
          {selected.entity_type === 'SELLER' && CAN_WRITE.includes(user?.role) && (
            <div style={{ marginTop: 24, padding: 16, border: '1px solid #e2e8f0', borderRadius: 8 }}>
              <h4 style={{ margin: '0 0 12px 0' }}>Invoice Configuration (Letterhead)</h4>
              <p style={{ fontSize: 13, color: '#666', marginBottom: 12 }}>Configure the logo and signatory details that will appear on invoices generated by this seller.</p>
              <InvoiceConfigForm 
                initialJson={selected.invoice_template_json} 
                onSave={handleInvoiceTemplateSave} 
              />
            </div>
          )}
