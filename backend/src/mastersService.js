...
const DEFAULT_PARAMS = [
  ...
  { category: 'BILLING', param_key: 'due_date_counting_mode', param_value: 'WORKING_DAYS', data_type: 'TEXT', unit: '', description: 'How payment terms are counted. WORKING_DAYS = N working days counted from the bill date (skips weekly offs and public holidays). CALENDAR_ROLL_FORWARD = bill date + N calendar days, then moved to next working day.' },
  ...
  { category: 'REGULATORY', param_key: 'certificate_price_bands', param_value: JSON.stringify({ REC: { floor: 0, forbearance: 2400 }, ESCERT: { floor: 1000, forbearance: 4000 } }), data_type: 'JSON', unit: 'INR/certificate', description: 'Floor and forbearance (ceiling) price per certificate, as set by the prevailing CERC order for RECs and by BEE/CERC for ESCerts. A null forbearance means no ceiling has been recorded — the bid screen then says the ceiling is not configured rather than accepting any price. Update these whenever a new order is issued; they are not derivable.' },
  { category: 'REGULATORY', param_key: 'escert_registry_available', param_value: '4250', data_type: 'NUMBER', unit: 'certificates', description: 'Desk snapshot of ESCert registry holdings available to sell until live registry is wired. Used by ECERTS Bid Entry sell-side balance checks.' },
  { category: 'REGULATORY', param_key: 'rec_registry_available', param_value: '85000', data_type: 'NUMBER', unit: 'certificates', description: 'Desk snapshot of REC registry holdings available to sell until live registry is wired. Used by REC Bid Entry sell-side balance checks.' },
  ...
  { category: 'TRADING', param_key: 'iex_base_url', param_value: '', data_type: 'TEXT', unit: '', description: 'Overrides the built-in IEX host for EVERY segment at once — normally blank. IEX serves each segment from its own host (iDAM, HPDAM, RTM, REC), and those are built into the client and selected by iex_environment. Set this only to point at a mock.' },
  { category: 'TRADING', param_key: 'iex_timeout_ms', param_value: '40000', data_type: 'NUMBER', unit: 'ms', description: 'Per-request timeout for IEX calls. The API spec fixes 40000 ms for every endpoint; lower it only if the exchange is observed hanging for the full window. Read per request, so a change takes effect without a restart.' },
  { category: 'TRADING', param_key: 'iex_rec_api_token', param_value: '', data_type: 'TEXT', unit: '', description: "Token for the REC/EC (ESCerts) API, if the exchange issues a separate one. Blank falls back to iex_api_token. IEX has not confirmed whether REC/EC shares the Front Office token — the one issued to SJVN carries the claim system='TradeV2Api'." },
  { category: 'TRADING', param_key: 'iex_enforce_token_expiry', param_value: 'false', data_type: 'TEXT', unit: '', description: "Whether a token whose own JWT expiry has passed is refused before the request is sent. IEX put token life at six months and called the one-hour expiry on SJVN's first UAT token a typo, so the default is to warn and still send. Set 'true' to re-arm the hard refusal." },
  ...
  { category: 'TRADING', param_key: 'iex_bid_area_id', param_value: 'ALL', data_type: 'TEXT', unit: '', description: "IEX BidAreaId used in the schedule and PQ reports. 'ALL' covers every bid area; set a specific id from the Bid Area Master to report one region." },
  { category: 'TRADING', param_key: 'iex_portfolio_id', param_value: 'ALL', data_type: 'TEXT', unit: '', description: "IEX PortfolioId used in the portfolio schedule report. 'ALL' covers every portfolio under the participant." },
  { category: 'TRADING', param_key: 'iex_environment', param_value: 'UAT', data_type: 'TEXT', unit: '', description: 'Which IEX environment to call — UAT (alpha) or LIVE. Selects the per-segment host set, and labels the data on screen so a UAT figure is never mistaken for a traded one.' },
