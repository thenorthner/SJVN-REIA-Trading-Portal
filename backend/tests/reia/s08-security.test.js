...
describe('S8 Payment security', () => {
  ...
  it('refuses release while dues or disputes are open', async () => {
    ...
    db.prepare(`INSERT INTO disputes (id, invoice_id, raised_by_entity_id, category, disputed_amount, status)
                VALUES ('DSP-OPEN', NULL, NULL, 'OTHER', 1000, 'RAISED')`).run();
    ...
    expect(r.status >= 400 || released !== 'APPROVED', 'security released with an open dispute outstanding').toBe(true);
  });
});
