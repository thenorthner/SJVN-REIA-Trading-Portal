...
def check():
    ...
    if pid == 0:
        ...
        commands = [
            "echo '=== SJVN PLATFORM STATUS ==='",
            f"echo '{REMOTE_PWD}' | sudo -S systemctl status sjvn-platform --no-pager",
            "echo '=== CURL LOCAL 4000 ==='",
            "curl -v http://127.0.0.1:4000/api/health",
            "echo '=== APACHE ERROR LOG ==='",
            f"echo '{REMOTE_PWD}' | sudo -S tail -n 20 /var/log/apache2/sjvn_error.log",
            "echo '=== APACHE DEFAULT ERROR LOG ==='",
            f"echo '{REMOTE_PWD}' | sudo -S tail -n 20 /var/log/apache2/error.log",
            "exit"
        ]
